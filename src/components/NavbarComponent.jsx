...
const NavbarComponent = ({ onChangeHandler, searchQuery, paramsNow }) => {
  const listMenu = [
    { name: "Home", to: "/" },
    { name: "Tambah Catatan", to: "/add" },
  ];
            ...
            {listMenu.map((menu, key) => (
              <li key={key}>
                <Link
                  to={menu.to}
                  className={
                    paramsNow.pathname === menu.to
                      ? `block py-2 px-3 text-white bg-blue-700 rounded-sm md:bg-transparent md:text-blue-700 md:p-0 md:dark:text-blue-500`
                      : `block py-2 px-3 text-gray-900 rounded-sm hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 md:p-0 md:dark:hover:text-blue-500 dark:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent`
                  }
                >
                  {menu.name}
                </Link>
